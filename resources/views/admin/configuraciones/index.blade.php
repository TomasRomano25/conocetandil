...
<div class="max-w-3xl space-y-3">
    ...
    {{-- ══════════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════════ --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <button type="button" onclick="toggleSection('planner-hero')"
            class="w-full px-6 py-4 flex items-center gap-3 text-left hover:bg-gray-50 transition group">
            <div class="w-9 h-9 bg-[#2D6A4F]/10 rounded-lg flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-[#2D6A4F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-[#1A1A1A]">Imagen de fondo — Planificador</p>
                <p class="text-xs text-gray-500 mt-0.5">Imagen de fondo del hero en la página del Planificador Premium</p>
            </div>
            @if($plannerHero)
            ...
            @endif
            <svg id="chevron-planner-hero" class="w-4 h-4 text-gray-400 flex-shrink-0 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
            </svg>
        </button>

        <div id="section-planner-hero" class="hidden border-t border-gray-100">
            <div class="px-6 py-5 space-y-5">
                <p class="text-xs text-gray-500">Esta imagen se muestra como fondo sutil en el hero del Planificador Premium. Se recomienda una foto horizontal de alta resolución (mínimo 1400px de ancho). Formatos: JPG, PNG, WebP. Máximo 4 MB.</p>

                {{-- Preview --}}
                @if($plannerHero)
                <div class="relative rounded-xl overflow-hidden border border-gray-200 bg-gray-50" style="height: 160px;">
                    <img src="{{ Storage::url($plannerHero) }}" alt="Hero actual"
                ...
                {{-- Upload --}}
                <form method="POST" action="{{ route('admin.configuraciones.planner-hero.update') }}" enctype="multipart/form-data" class="flex items-end gap-3">
                    @csrf
                    <div class="flex-1">
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">{{ $plannerHero ? 'Reemplazar imagen' : 'Subir imagen' }}</label>
                ...
                {{-- Delete --}}
                @if($plannerHero)
                <form method="POST" action="{{ route('admin.configuraciones.planner-hero.delete') }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        onclick="return confirm('¿Eliminar la imagen del hero del Planificador?')"
