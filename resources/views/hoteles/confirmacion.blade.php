@extends('layouts.app')
@section('title', 'Hotel registrado — Conoce Tandil')
...
<div class="max-w-lg mx-auto px-4 sm:px-6">
    ...
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-[#1A1A1A]">¡Hotel registrado!</h1>
        <p class="text-gray-500 mt-2">
            Revisaremos tu hotel y lo publicaremos <strong>dentro de las próximas 24 horas</strong>.
        </p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-7 mb-5">
        ...
        <div class="space-y-3 text-sm">
            <div class="flex justify-between">
                ...
                <span class="font-semibold text-[#1A1A1A]">{{ $order->hotel->name }}</span>
            </div>
            <div class="flex justify-between">
        ...
        <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between py-1.5 border-b border-gray-50">
                ...
                <button onclick="copyText('{{ $bankConfig['cbu'] }}', this)"
                    class="font-mono font-semibold text-[#2D6A4F] hover:underline">{{ $bankConfig['cbu'] }}</button>
            </div>
            @endif
            @if ($bankConfig['alias'])
            <div class="flex justify-between py-1.5">
                ...
                <button onclick="copyText('{{ $bankConfig['alias'] }}', this)"
                    class="font-semibold text-[#2D6A4F] hover:underline">{{ $bankConfig['alias'] }}</button>
            </div>
            @endif
        </div>
    ...
    <div class="space-y-3 mb-8">
        @foreach([
            ['✅', 'Hotel registrado', 'Tu solicitud fue guardada correctamente.'],
            ['🕐', 'Verificación en proceso', 'Revisaremos tu hotel y el pago en las próximas horas.'],
            ['🏨', 'Publicado en el directorio', 'Una vez aprobado, tu hotel aparecerá en la búsqueda.'],
        ] as [$icon, $title, $desc])
